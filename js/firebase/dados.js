...
async function carregaDadosAlteracao(db, id) {
  await firebase.database().ref(db + '/' + id).on('value', (snapshot) => {
    ...
    document.getElementById('dcompra').value = snapshot.val().dcompra // --------------------alterado o ID.. dcompra
    document.getElementById('pcusto').value = snapshot.val().pcusto // --------------------alterado o ID.. pcompra
    document.getElementById('pvenda').value = snapshot.val().pvenda // --------------------alterado o ID.. pvenda
...
function salvar(event, collection) {
  ...
  else if (document.getElementById('nfe').value < 0) { alerta('⚠️A nota fiscal não pode ser negativa', 'warning') } // caso informe nf negativa
  else if (document.getElementById('dcompra').value === '') { alerta('⚠️É obrigatório informar a data da compra!', 'warning') }
  else if (document.getElementById('dcompra').value < 0) { alerta('⚠️Preço de compra não pode ser negativo', 'warning') } // caso informe preço de compra negativo
  else if (document.getElementById('pcusto').value === '') { alerta('⚠️É obrigatório informar o preço de custo!', 'warning') } // alterei para ficar somente se for vazio
  else if (document.getElementById('pcusto').value < 0) { alerta('⚠️Preço de custo não pode ser negativo', 'warning') } // caso informe preço de custo negativo
  else if (document.getElementById('pvenda').value === '') { alerta('⚠️É obrigatório informar o preço de venda!', 'warning') } // alterei para ficar somente se for vazio
  else if (document.getElementById('pvenda').value < 0) { alerta('⚠️Preço de venda não pode ser negativo', 'warning') } // caso informe preço de venda negativo
